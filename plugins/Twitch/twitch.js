		...
		if(options.command == "online") {
			if(result.stream) {
				...
				for(i in command_api.fields) {
					var replace = "";
					if(command_api.fields[i].indexOf('.') >= 0) {
						var subfields = command_api.fields[i].split('.');
						replace = result.stream[subfields[0]][subfields[1]];
					} else {
						replace = result.stream[command_api.fields[i]];
					}
					message = message.replace("#"+command_api.fields[i]+"#", replace);
				}
